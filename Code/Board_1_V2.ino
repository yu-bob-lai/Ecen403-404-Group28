...
void loop(void){
  ...
  if(mySerial.available()){
   ...
   //210  pot on
    while (mode == 0){
        ...
        if (state > 200){
           if(state==209){ // pot off
            mode = 1; 
          }
          else if(state==210){ // current set
            mode = 1;
            state = 224; 
          }
          else if (state == 211){
 ...
 Serial.print("Current (pot): ");
  ...
  Serial.print("Voltage (pot): ");
Serial.println(voltageValue, 3);
  Serial.print("Power (pot): ");
...
while (mode == 1){
         ...
         if(state == 2){ // pot ON
          ...
          while(state == 224 || state == 225){
            if(mySerial.available()){
              ...
              else if(x == 2){ // turn the original switch ON
                if(state == 224){
                state = 30;
                mode = 0;
                }
  ...
  Serial.println(currentValue, 3);
  Serial.print("Voltage (app): ");
Serial.println(voltageValue, 3);
  Serial.print("Power (app): ");
Serial.println(powerValue, 3);
  Serial.print("ModeA: ");
Serial.println(mode);
  Serial.print("StateA: ");
